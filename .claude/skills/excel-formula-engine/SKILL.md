...
description: "Cria, corrige, replica e audita fórmulas do Excel preservando referências e lógica."
---

# Excel Formula Engine

Cria, corrige, replica e audita fórmulas do Excel preservando referências e lógica.
...
## Regras de fórmula
- Preferir referências a células/parâmetros em vez de números mágicos.
- Validar `$` em referências absolutas/mistas antes de preencher.
- Usar tabela estruturada quando isso melhorar legibilidade e expansão.
- Ao corrigir uma fórmula, comparar padrão das linhas vizinhas e intenção da coluna.
- Não preencher fórmula sobre linha de subtotal/cabeçalho.
- Evitar fórmulas voláteis desnecessárias.

## Auditoria
- Detectar fórmulas divergentes dentro da mesma coluna lógica.
- Identificar `#REF!`, intervalos truncados, off-by-one e referências para abas inexistentes.
- Validar divisão por zero e blanks.
- Verificar se totals/subtotals cobrem o conjunto correto.

## Saída
Para cada correção relevante registrar `célula/range`, `antes`, `depois`, `motivo` e `evidência`.
